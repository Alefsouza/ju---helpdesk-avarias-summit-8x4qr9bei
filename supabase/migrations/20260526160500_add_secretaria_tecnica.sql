...
ALTER TABLE public.documentos ADD COLUMN IF NOT EXISTS orcamento_url TEXT;
...
BEGIN
  ...
  CREATE POLICY "documentos_update" ON public.documentos
    ...
    USING (
      ((chamado_id IS NULL) OR (chamado_id IN ( SELECT chamados.id FROM chamados WHERE ((chamados.responsavel_id = auth.uid()) OR (chamados.usuario_id = auth.uid())))) OR is_admin() OR is_responsavel() OR is_sinistro() OR is_vistoriador() OR is_juridico() OR public.is_secretaria_tecnica())
    )
    WITH CHECK (
      ((chamado_id IS NULL) OR (chamado_id IN ( SELECT chamados.id FROM chamados WHERE ((chamados.responsavel_id = auth.uid()) OR (chamados.usuario_id = auth.uid())))) OR is_admin() OR is_responsavel() OR is_sinistro() OR is_vistoriador() OR is_juridico() OR public.is_secretaria_tecnica())
    );
